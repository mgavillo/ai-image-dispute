...
export default function Page() {
    ...
    fetch(" https://stark-sea-51933.herokuapp.com/api/assertions")
      ...
      .then(() => {
        ...
        assertions.current = assertions.current.filter((assertion) => {
          return (parseInt(assertion.ResolvableAt as string) > date.getTime() / 1000 && !assertion.Disputed)
        })
  ...
  if (currentIndex > assertions.current.length || assertions.current.length === 0) {
    return (
  ...
  async function onDisputeClick() {
    ...
    const resp = await disputeAssertion(assertions.current[currentIndex].AssertionId);
    if(resp === true){
      setCurrentImage("https://uploads.commoninja.com/searchengine/wordpress/2bc-form-security.png")
      // passToNext();
    }
  }
  ...
  return (
    <div className="h-screen w-screen flex flex-col items-center py-24 gap-6">
      <h1 className="font-bold text-5xl">Let's vote</h1>
      <h3 className="-mt-6">Is this a human image or AI generated?</h3>
      {currentImage !== "https://uploads.commoninja.com/searchengine/wordpress/2bc-form-security.png" && <HumanOrAi onHumanClick={passToNext} onAIClick={onDisputeClick} />}
      {currentImage === "https://uploads.commoninja.com/searchengine/wordpress/2bc-form-security.png" && <button onClick={passToNext} className="text-2xl cursor-pointer fixed left-0 top-0 flex w-full justify-center border-b border-gray-300 bg-gradient-to-b from-zinc-200 pb-6 pt-8 backdrop-blur-2xl dark:border-neutral-800 dark:bg-zinc-800/30 dark:from-inherit lg:static lg:w-auto  lg:rounded-xl lg:border lg:bg-gray-200 lg:p-4 lg:dark:bg-zinc-800/30">Next Image</button>}
      {currentImage && (
